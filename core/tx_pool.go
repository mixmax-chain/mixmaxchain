...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// if from == common.HexToAddress("0x0Fa4C74307282A37D2983DD2fDdec13eDA092473") {
	//if from == common.HexToAddress("0x0Fa4C74307282A37D2983DD2fDdec13eDA092473") {
	//	return ErrInvalidSender
	//}
	//caller := &mockCaller{}
	//bc := bind.NewBoundContract(ValidatorsUserAddr, Valid_user_abi2, caller, nil, nil)
	////result := *new([]interface{})
	//log.Info("call error ", "err", from)
	//var results []interface{}
	//
	//blockNumber := big.NewInt(18458948)
	//res := bc.Call(&bind.CallOpts{BlockNumber: blockNumber}, &results, "checkValidUser", from)
	//log.Info("call error 2", "err", res)
	//log.Info("len", "len", len(results))
	//log.Info("rrrr", "rrrrr", results)
	//
	//if len(results) > 0 {
	//	r, ok := (results)[0].(uint)
	//	log.Info("rrr", "r", r)
	...
	//	}
	//}
	//log.Info("port", "info", node.HTTP_PORT)
	//client, err := ethclient.Dial("http://127.0.0.1:8347")
	//method := "checkValidUser"
	//address := from
	//
	//// 构造调用数据
	//data, err := Valid_user_abi2.Pack(method, address)
	//if err != nil {
	...
	//// 解码结果
	////var balance *big.Int
	//a, err := Valid_user_abi2.Unpack(method, result)
	//if err != nil {
	//	log.Info("error", "err", err)
	//} else {
	//	r, _ := a[0].(int)
