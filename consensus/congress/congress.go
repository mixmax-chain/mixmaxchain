	...
	if c.chainConfig.RedCoastBlock != nil && c.chainConfig.RedCoastBlock.Cmp(header.Number) < 0 {
		if header.Number.Cmp(big.NewInt(22325300)) > 0 {
			if fee.Cmp(common.Big0) > 0 {
				...
				if err == nil {
					burnFee := new(big.Int).Mul(fee, big.NewInt(burnRate))
					burnFee = new(big.Int).Div(burnFee, big.NewInt(1000))
					state.AddBalance(common.HexToAddress("0x0000000000000000000000000000000000000001"), burnFee)
					distributeFee := new(big.Int).Sub(fee, burnFee)
					fee = distributeFee
					distributeRate, _ := dr.GetDistributeRate(state, header, newChainContext(chain, c), c.chainConfig)
					if distributeRate > 0 {
						distributeNode := new(big.Int).Mul(distributeFee, big.NewInt(distributeRate))
						distributeNode = new(big.Int).Div(distributeNode, big.NewInt(1000))
						fee = distributeNode
						distributeOther := new(big.Int).Sub(distributeFee, distributeNode)
						state.AddBalance(dr.GetAddress(), distributeOther)
					} else {
						state.AddBalance(dr.GetAddress(), fee)
						fee = common.Big0
			...
			fee = new(big.Int).Add(fee, big.NewInt(1_000_000_000_000_000_000)) // 1 ether
		}
	}
