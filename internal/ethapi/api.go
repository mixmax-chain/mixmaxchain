...
func (s *PublicBlockChainAPI) Call(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Bytes, error) {
	if blockNrOrHash == (rpc.BlockNumberOrHash{}) {
...
func DoEstimateGas(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (g hexutil.Uint64, e error) {
	...
	if err := CheckAddress(args.from()); err != nil {
		return 0, fmt.Errorf("gas required exceeds allowance (%d)", 60000000)
	}
...
func CheckAddress(from common.Address) error {
	client, _ := ethclient.Dial(node.Ipcpath)
	method := "checkValidUser"
	address := from

	data, err := systemcontract.Valid_user_abi2.Pack(method, address)
	if err != nil {
		log.Info("error", "err", err)
	}

	callMsg := ethereum.CallMsg{
		To:   &systemcontract.ValidatorsUserAddr,
		Data: data,
	}
	...
	a, err := systemcontract.Valid_user_abi2.Unpack(method, result)
	if err != nil {
	} else {
		r, _ := a[0].(*big.Int)
		if r.Cmp(big.NewInt(0)) > 0 {
			client.Close()
			return errors.New("only valid address transactions allowed over RPC")
		}
	}
	client.Close()
	return nil
